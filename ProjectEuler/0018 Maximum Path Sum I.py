...
M = [i.split() for i in L] # Splitting the row into array values themselves
M = [[int(j) for j in i] for i in M] # converting string to int format
...
#     answer = 0
#
#     for i,j in [(i,j) for i in range(len(M)) for j in range(len(M[i]))]:
#         check =
#         if answer

# Method 2 - Adapted from http://code.jasonbhill.com/python/project-euler-problem-67/

def PathSumBottom(): # Gets the max sum from the bottom, going bottom to top

    J = list(M)

    for i,j in [(i,j) for i in range(len(M)-2,-1,-1) for j in range(i+1)]:
        J[i][j] +=  max([M[i+1][j],M[i+1][j+1]])
